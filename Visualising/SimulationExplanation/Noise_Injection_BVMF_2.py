...
ax0.contour(X,Y, z, t_contours, colors="green", zorder = 2)


x, y = zip(*Gen.detected_coords)
for i, (x, y, s) in enumerate(zip(x, y, Gen.detected_luminosities)):
    if s == Gen.BH_true_luminosities[0]:
        ax0.add_artist(plt.Circle(xy=(x, y), radius=s+0.001*Gen.L_star, color="green", zorder = 3))

x = np.linspace(u_r - 5*np.sqrt(u_r), u_r + 5*np.sqrt(u_r))
pdf = Gen.burr(x, c = Gen.BVM_c, k = Gen.BVM_k, l = u_r)
# Plotting
ax1.plot(x, pdf, 'r-', lw=2)
ax0.scatter(Gen.BH_detected_coords[0,0], Gen.BH_detected_coords[0,1], s = 200, color = 'red', marker = 'x', zorder = 5)
