...
if events * 10 > total_luminosity / (characteristic_Luminosity*alpha):
...
for Universe in range(universe_count):
    Gen = EventGenerator(dimension=dimension, size=size, event_count=events,
                         ...
                         cluster_coeff=5, characteristic_luminosity=characteristic_Luminosity, total_luminosity=total_luminosity, alpha=alpha,
                         event_distribution="Proportional", noise_distribution="BVM", noise_std=3, contour_type="BVM", redshift_noise_sigma=0,
    ...
    H_0_samples[Universe] = H_0_sample/np.sum(H_0_sample)
