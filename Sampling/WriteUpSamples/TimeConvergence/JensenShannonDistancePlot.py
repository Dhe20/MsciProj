...
for i, file in enumerate(csv_list):
    ...
    for j, column in enumerate(df.columns):
        pdf_single = df[column]/df[column].sum()
        posterior_converging[j, i, :] = pdf_single

js_distance_arr = np.zeros((number_of_universes, number_of_time_samples-1))

for i, matrix in enumerate(posterior_converging):
    if np.isnan(matrix).any():
        js_distance_arr[i, :] = np.full(fill_value= np.nan ,shape=number_of_time_samples-1)
        continue
    for posterior_number, posterior in enumerate(matrix):
        if posterior_number == number_of_time_samples-1:
            continue
        js_distance_arr[i,posterior_number] = scipy.spatial.distance.jensenshannon()
...
# js_distance_arr = np.sqrt(js_distance_arr[np.logical_not(np.isnan(js_distance_arr).any(axis=1)), :])
# df = pd.DataFrame(js_distance_arr, columns = [str(num) for num in nums[:-1]])
# df2 = df.stack().reset_index()
# df2.columns = ['Universe_Number','AvgEventNum','JSDiv']

#
# g = (
# ggplot(df2, aes(x = 'AvgEventNum', y = 'JSDist', group = 'AvgEventNum')) +
#   geom_boxplot(fill="lightskyblue")
# # + scale_y_continuous(trans='log10')
# + xlab("Average Event Number (Equal Time Increment)")
