...
path = '/Users/daneverett/PycharmProjects/MSciProject/Sampling/WriteUpSamples/Clustering_Realistic_Investigation/'
...
investigated_characteristic = "20LocVol_realistic_clustering"
# N_centroids = [10,15,20,25]#,20,25
investigated_values = [0.0005, 0.001, 0.005 ,0.01,0.05, 0.1,0.5,1,]
investigated_values = sorted(investigated_values, reverse=True)
...
max_numbers = ["1"]*(len(investigated_values)+1)
event_count_max_numbers = ["1"]*(len(investigated_values)+1)
...
for axis_third, investigated_characteristic in enumerate(["10LocVol_realistic_clustering", "20LocVol_realistic_clustering"]):
    if axis_third==0:
        max_numbers = ["1"] * (len(investigated_values) + 1)
    else:
        max_numbers = ["1"] * (len(investigated_values) + 1)
    ...
    for i in range(len(investigated_values)):
        # print(i)
        filename = path + "SampleUniverse_" + str(investigated_characteristic) + "_" +str(investigated_values[i]) + "_" + \
                   max_numbers[i] + ".csv"
        df = pd.read_csv(filename, index_col=0)
        df.dropna(inplace=True, axis=1)
        filename = path + "EventCount_SampleUniverse_" + str(investigated_characteristic) + "_" +str(investigated_values[i]) + "_" + \
        ...
        if i == 0:
            good_samples = list(np.where(df_event_count.values[0]>=20)[0])
    ...
    ax[0,axis_third].errorbar(investigated_values, [np.mean(alphas) for alphas in alphass],
                   yerr = [np.std(alphas)/np.sqrt(len(stds)) for alphas in alphass], ls = "--", fmt = 'v',
               )
    ax[1,axis_third].errorbar(investigated_values[0:len(investigated_values)], [np.mean(stds) for stds in stdss],
                   yerr = [np.std(stds)/np.sqrt(len(stds)) for stds in stdss], ls = "--", fmt = 'v',
                    )
