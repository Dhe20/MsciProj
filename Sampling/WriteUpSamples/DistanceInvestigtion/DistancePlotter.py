...
for j in range(len(investigated_values)):
    c = next(color)
    edgecolor='white'
    ax.scatter(np.array(Ns), np.array(sigmas_s)[:,j]/70, marker='^', s=100, c=c, label=r'$\sigma_D/D = {}\% $'.format(rel[j]), zorder=3)
    c[3] = 0.7
...
#ax.grid(axis='both', ls='dashed', alpha=0.5)
ax.tick_params(axis='both', which='major', direction='in', labelsize=30, size=8, width=3, pad = 9)
ax.legend(fontsize = 20, loc='center right', framealpha=1)
ax.set_ylabel(r'$\sigma_{H_0}/H_0$ (km s$^{-1}$ Mpc$^{-1}$)', fontsize=35, labelpad=15)
ax.set_xlabel(r'$\hat N$', fontsize=35, labelpad=15)
#ax.set_ylim(-0.01,0.2)
ax.set_xlim(4,1100)
ax.set_yscale('log')
ax.set_xscale('log')
...
for j in range(len(Ns)):
    ...
    if Ns[j] == 100 or Ns[j] == 200:
        ...
        ax.scatter(np.array(investigated_values), np.array(sigmas_s[j])/70, marker='^', s=100, c=c, label=r'$\bar N = {}$'.format(Ns[j]), zorder=3)
        c[3] = 0.7
